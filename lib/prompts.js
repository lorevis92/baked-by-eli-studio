...
// Nota di ancoraggio: usata quando inviamo DUE immagini a un modello
// di edit — [0] = foto originale intatta, [1] = versione corrente da
// correggere. Serve a impedire che la torta "derivi" leggermente ad
// ogni passaggio della pipeline (drift cumulativo su step concatenati).
const ANCHOR_NOTE = `
Ti fornisco due immagini:
- PRIMA immagine: la foto ORIGINALE della torta, non ancora modificata. È il riferimento assoluto per l'identità della torta (forma, decorazioni, colori, scritte). NON deriva da essa nessuna modifica.
- SECONDA immagine: la versione attualmente in lavorazione, su cui applicare la correzione richiesta qui sotto.

Prima di editare, confronta la torta nella SECONDA immagine con quella nella PRIMA: se noti una qualunque differenza non intenzionale nella forma, nei colori o nelle decorazioni rispetto all'originale, correggila riportandola fedele alla PRIMA immagine, mentre applichi la modifica richiesta.`;

export function buildPrompt(step, { format = '1:1', feedback = [], anchored = false } = {}) {
  let p = step.prompt.replaceAll('{FORMAT}', FORMATS[format] || FORMATS['1:1']);
  if (anchored) p = ANCHOR_NOTE + '\n\n' + p;
...
export function buildFeedbackPrompt(feedbackText) {
  return (
    ANCHOR_NOTE +
    '\n\n' +
    FEEDBACK_STEP.prompt.replaceAll('{FEEDBACK}', feedbackText.trim())
  );
}
